...
// This function iterates through all emergency centers, calculating the distance to the site,
// and selects the center with the smallest distance.
...
// This function implements Dijkstra's algorithm, which works as follows:
// 1. Initialize distances to all nodes as infinity, except the start node, which is set to 0.
// 2. Use a priority queue to repeatedly select the node with the smallest tentative distance.
// 3. Update the distances of the current node's neighbors if a shorter path is found.
// 4. Mark the current node as visited to ensure it is not processed again.
// 5. Repeat until all nodes have been visited or the queue is empty.
...
// ReconstructPath reconstructs the shortest path from start to target using the previous map generated by Dijkstra's algorithm.
...
// It connects each telecom site to its nearest hub, each hub to its two nearest hubs,
// and each emergency center to the five nearest hubs.
